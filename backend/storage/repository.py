...
class Repository:
        ...
        with get_session() as s:
            s.merge(
                TradeIdeaModel(
                    ...
                    llm_confidence_scale=llm_analysis.confidence_scale
    ...
    def clear_pending_approvals(self) -> int:
        ...
        with get_session() as s:
            rows = s.query(ApprovalRequestModel).filter(ApprovalRequestModel.status == "pending").all()
            for row in rows:
                row.status = "cleared"
            return len(rows)
        ...
        with get_session() as s:
            ...
            for row in rows:
                ...
                idea = TradeIdea(
                    ...
                    mode_eligibility=[TradingMode(mode) for mode in (idea_row.mode_eligibility or [])],
                )
                risk = RiskDecision(
    ...
    def get_pending_live_orders(self) -> list:
        ...
        with get_session() as s:
            ...
            return [{"id": r.id, "exchange_order_id": r.exchange_order_id, "price": r.price or 0.0} for r in rows]

    def update_order_status_and_fill(self, order_id: str, status: str, fill_price: float, fee: float) -> None:
    ...
    def get_signal_outcomes(self, strategy_id: Optional[str] = None, limit: int = 1_000) -> List[SignalOutcomeModel]:
        with get_session() as s:
    ...
    def upsert_news_item(
        self, *, id: str, source: str, title: str, content: str, published_at: datetime, url: Optional[str]
    ) -> None:
        with get_session() as s:
    ...
    def get_recent_news(self, limit: int = 60) -> List[Dict]:
        ...
        with get_session() as s:
            rows = s.query(NewsItemModel).order_by(NewsItemModel.published_at.desc()).limit(limit).all()
            return [
                {
                    "id": r.id,
                    "source": r.source,
    ...
    def save_equity_snapshot(self, total_equity: float, cash: float, positions_value: float) -> None:
        with get_session() as s:
            s.add(
                EquitySnapshotModel(
    ...
    def get_equity_history(self, limit: int = 288) -> List[Dict]:
        ...
        with get_session() as s:
            rows = s.query(EquitySnapshotModel).order_by(EquitySnapshotModel.timestamp.desc()).limit(limit).all()
            return [{"timestamp": r.timestamp.isoformat(), "equity": r.total_equity} for r in reversed(rows)]

    def get_latest_equity(self) -> Optional[float]:
        with get_session() as s:
            row = s.query(EquitySnapshotModel.total_equity).order_by(EquitySnapshotModel.timestamp.desc()).first()
            return row[0] if row else None

    def get_latest_cash(self) -> Optional[float]:
        """Return the last persisted cash balance (not total equity)."""
        with get_session() as s:
            row = s.query(EquitySnapshotModel.cash).order_by(EquitySnapshotModel.timestamp.desc()).first()
            return row[0] if row else None
    ...
    def save_activity_log(self, level: str, message: str, detail: str = "") -> None:
        with get_session() as s:
            s.add(ActivityLogModel(level=level, message=message, detail=detail, timestamp=datetime.now(timezone.utc)))
    ...
    def get_recent_activity(self, limit: int = 200) -> List[Dict]:
        ...
        with get_session() as s:
            rows = s.query(ActivityLogModel).order_by(ActivityLogModel.timestamp.desc()).limit(limit).all()
            return [
                {"timestamp": r.timestamp.isoformat(), "level": r.level, "message": r.message, "detail": r.detail or ""}
                for r in rows
            ]
    ...
    def trim_old_activity(self, keep: int = 2_000) -> None:
        ...
        with get_session() as s:
            ...
            if count > keep:
                ...
                if cutoff_id:
                    s.query(ActivityLogModel).filter(ActivityLogModel.id <= cutoff_id).delete(synchronize_session=False)
    ...
    def get_recent_risk_rejections(self, limit: int = 50) -> List[Dict]:
        ...
        with get_session() as s:
            rows = s.query(RiskRejectionModel).order_by(RiskRejectionModel.timestamp.desc()).limit(limit).all()
            return [
                {
                    "market": r.market,
                    "direction": r.direction,
    ...
    def get_rejected_trades(self, limit: int = 100) -> List[Dict]:
        ...
        with get_session() as s:
            rows = s.query(RejectedTradeModel).order_by(RejectedTradeModel.timestamp.desc()).limit(limit).all()
            trade_idea_ids = {r.trade_idea_id for r in rows if r.trade_idea_id}
            strategy_by_idea = {}
            if trade_idea_ids:
                strategy_rows = (
    ...
    def save_control_state(
        ...
        selected_strategy: str = "basic_and_llm_strategy",
    ) -> None:
    ...
    def load_control_state(self) -> Optional[Dict]:
        ...
        with get_session() as s:
            ...
            return {
                ...
                "selected_strategy": row.selected_strategy or "unknown",
            }

    # ── LLM briefings ─────────────────────────────────────────────────────

    def save_llm_briefing(
        self,
        *,
        key_insight: str,
        overall_sentiment: float,
        market_outlooks: dict,
        article_count: int,
    ...
    def load_latest_llm_briefing(self) -> Optional[Dict]:
        with get_session() as s:
            row = s.query(LLMBriefingModel).order_by(LLMBriefingModel.generated_at.desc()).first()
            if row is None:
                return None
            return {
    ...
    def save_llm_reflection(
        self, *, pattern: str, suggestion: str, insight_confidence: float, generated_at: Optional[datetime] = None
    ) -> None:
        with get_session() as s:
            s.add(
                LLMReflectionModel(
    ...
    def load_latest_llm_reflection(self) -> Optional[Dict]:
        with get_session() as s:
            row = s.query(LLMReflectionModel).order_by(LLMReflectionModel.generated_at.desc()).first()
            if row is None:
                return None
            return {
                "pattern": row.pattern,
    ...
    def save_risk_state(self, *, daily_loss: float, daily_start_equity: float, last_reset_date) -> None:
        """Upsert the single-row risk state so daily_loss survives a restart."""
        from datetime import date as date_type

        date_str = last_reset_date.isoformat() if isinstance(last_reset_date, date_type) else str(last_reset_date)
        with get_session() as s:
            s.merge(
                RiskStateModel(
                    id=1,
